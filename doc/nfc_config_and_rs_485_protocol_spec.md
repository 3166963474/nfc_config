...
| ----------:| ---:| ---------- | ----------- |
| 0          | 8   | rf         | LoRa RF 参数  |
| 8          | 16  | reserved   | 硬件配置/保留段；bit2 可配置无源蜂鸣器 |
| 24         | 1   | vehicle_id | 车辆号         |
...
`reserved[0]` 的已定义硬件配置位：

| bit | 值为 1 时的含义 |
| ---: | --- |
| 0 | 座椅占用传感器高电平有效 |
| 1 | 安全带闭合传感器低电平有效 |
| 2 | 使用无源蜂鸣器：PB7 由 TIM4 CH2 PWM 驱动；0 表示有源蜂鸣器 GPIO 驱动 |

`reserved[1]` 和 `reserved[2]` 用于从机定时状态上报：

| 字节 | 配置范围 | 含义 |
| --- | --- | --- |
| `reserved[1]` | 0~255 | 基准上报间隔，单位 8 秒。值为 0 时关闭定时上报。 |
| `reserved[2]` | 0~255 | 随机延迟范围，单位秒；每次调度从 `0~reserved[2]`（含端点）随机选一个值。 |

启用时实际间隔为 `reserved[1] × 8 秒 + 随机值秒`。例如 `reserved[1]=6`、`reserved[2]=0` 时固定每 48 秒上报；`reserved[1]=6`、`reserved[2]=2` 时每次为 48、49 或 50 秒。

其余 `reserved` 字节及未定义 bit 填 0。
...
5. `reserved[0]` 的 bit0~bit2 为硬件配置位；从机的 `reserved[1]` 为定时上报基准间隔（单位 8 秒），`reserved[2]` 为随机延迟范围（单位秒）；其余 `reserved` 字节及未定义 bit 建议填 0。
